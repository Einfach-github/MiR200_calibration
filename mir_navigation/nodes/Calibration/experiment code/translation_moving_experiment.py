...
""" translation_moving_experiment.py : This program aims to control the mir_200 robot moving in translation. Move by publishing velocity command cmd_vel;
...
class TranslationMoving():
	"""class for moving mir_200 robot in translation"""
	def __init__(self):
		#Give a name for node
		rospy.init_node('translation_moving', anonymous=False)
		...
		self.translation_length = rospy.get_param("param_translation_length", 1.0)            # in meters
		...
		#Start moving robot in translation
		choice = self.choose()

		if (choice == 1):
			self.state = self.MovingRobot(self.translation_length, self.linear_velocity, self.translation_tolerance)
			if self.state == True:
				rospy.loginfo("translation moving has been completed")
			else:
				rospy.loginfo("translation moving failed")
		elif (choice == 0):
			exit(0)
	...
	def choose(self):
		...
		choice = 'a'
		rospy.loginfo("--------------------------------------------------------------------------")
		rospy.loginfo("choose 1: moving robot in 1m translation")
	...
	def MovingRobot(self, translation_length, linear_velocity, translation_tolerance):
		...
		for i in range(1):
			...
			#Enter the loop to move robot forward (1m translation by default)
			...
			#Stop the translation movement by publishing the zero value of velocity
			move_cmd.linear.x = 0
			move_cmd.linear.y = 0
			move_cmd.linear.z = 0
			move_cmd.angular.x = 0
		...
		#Stop the robot after translation movement by publishing the zero value of velocity
		#move_cmd = Twist()
		#move_cmd.linear.x = 0
		#move_cmd.linear.y = 0
		#move_cmd.linear.z = 0
		...
		#with open("real_pose_translation.txt", 'w') as f:
