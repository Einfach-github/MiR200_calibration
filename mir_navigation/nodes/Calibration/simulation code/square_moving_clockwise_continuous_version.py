...
""" square_moving_clockwise.py : This program aims to control the mir_200 robot moving in a square path. Move by publishing velocity command cmd_vel;
    stop publishing velocity command by checking odometry info from odom_comd; track the real pose by checking info from the base_ground_pose_truth topic.
    Written by Cui
"""
...
#For subscribing the topics odom_comb and base_pose_ground_truth using message type Odometry
from tf.transformations import euler_from_quaternion

class SquareMovingClockwise():
	"""class for moving mir_200 robot in clockwise direction with square path"""
	def __init__(self):
		#Give a name for node
		rospy.init_node('square_moving_clockwise', anonymous=False)
		...
		#Define variables for callback function of subscriber odom_comb
		self.x = 0
		self.y = 0
		self.theta = 0 
		...
		self.translation_length = rospy.get_param("param_translation_length", 4.0)            # in meters
		...
		self.rotation_tolerance = rospy.get_param("param_rotation_tolerance", radians(4.50))  # see below
		...
		#Define a subscriber with topic odom_comb for subscribing the wrong odometry of robot
		self.odom = rospy.Subscriber("/odom_comb", Odometry, self.OdometryCallback)
	...
	def choose(self):
		...
		choice = 'a'
		rospy.loginfo("--------------------------------------------------------------------------")
		rospy.loginfo("choose 1: moving robot with square path in clockwise direction")
	...
	def MovingRobot(self, translation_length, rotation_angle, linear_velocity, angular_velocity, translation_tolerance, rotation_tolerance):
		...
		for i in range(4):
			...
			#Set the angular velocity for translation in clockwise direction
			move_cmd.linear.x = 0
			move_cmd.linear.y = 0
			move_cmd.linear.z = 0
			move_cmd.angular.x = 0
			...
			#Enter the loop to turn robot in clockwise direction (90 rotation by default)
		...
		with open("real_pose_clockwise.txt", 'w') as f:
	...
	#Define a callback function for subscriber (topic: odom_comb)
